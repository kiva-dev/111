    ...
    <view class="register-box" v-if="Inv == 1">
      ...
      <view class="login-ul">
        ...
        <view class="login-li">
          ...
          <view class="li-input">
            <input class="input" type="password" placeholder-class="color-999" v-model="pwd" :placeholder="$t('login.qsrmm')" />
          </view>
        </view>
        <view class="login-li">
          <view class="label">{{$t('login.qrmm')}}</view>
          <view class="li-input">
            <input class="input" type="password" placeholder-class="color-999" v-model="pwd2" :placeholder="$t('login.qsrqrmm')" />
          </view>
        </view>
        <view class="login-li">
      ...
      <view class="login-ul">
        ...
        <view class="login-li">
          ...
          <view class="li-input">
            ...
            <view class="yzm" v-if="codeTxt==$t('login.hqyzm')" @click.stop="$noMultipleClicks(onLoginSendEmailCode)">{{$t('login.fsyzm')}}</view>
        ...
        <view class="login-li">
          ...
          <view class="li-input">
            <input class="input" type="password" maxlength="16" placeholder-class="color-999" v-model="pwd" :placeholder="$t('login.qsrmm')" />
          </view>
        </view>
        <view class="login-li">
          <view class="label">{{$t('login.qrmm')}}<text>*</text></view>
          <view class="li-input">
            <input class="input" type="password" maxlength="16" placeholder-class="color-999" v-model="pwd2" :placeholder="$t('login.qsrqrmm')" />
          </view>
        </view>
        <view class="login-li">
          <view class="label">{{$t('login.yqm')}}</view>
...
export default {
  ...
  methods: {
    ...
    onLoginMobileRegister () {
      ...
      if (!this.pwd2) return uni.showToast({ title: this.$t('login.qqrmm'), icon: 'none' })
      if (this.mobile_area_code == 86) {
        if (this.mobile) {
          var reg_tel = /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/ //11位手机号码正则
          if (!reg_tel.test(this.mobile)) return uni.showToast({ title: this.$t('login.qsrzqsjh'), icon: 'none' })
      ...
      if (this.pwd !== this.pwd2) return uni.showToast({ title: this.$t('login.lcmmbyzqcxsr'), icon: 'none' })
      if (!this.isQuanShow) return uni.showToast({ title: this.$t('login.qydxybty'), icon: 'none' })
    ...
    onLoginEmailRegister () {
      ...
      if (this.pwd) {
        ...
        if (!reg.test(this.pwd)) return uni.showToast({ title: this.$t('pwdcd'), icon: 'none' })
      }
      if (!this.pwd2) return uni.showToast({ title: this.$t('login.qqrmm'), icon: 'none' })
      if (this.pwd !== this.pwd2) return uni.showToast({ title: this.$t('login.lcmmbyzqcxsr'), icon: 'none' })
      if (!this.isQuanShow) return uni.showToast({ title: this.$t('login.qydxybty'), icon: 'none' })
