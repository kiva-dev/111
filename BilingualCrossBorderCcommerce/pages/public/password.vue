<template>
  <view class="register-page">
    ...
    <!-- <view class="register-tab">
      <view :class="['li',Inv==0?'active':'']" @click="changeTab(0)">{{$t('login.sjhyz')}}</view>
      <view :class="['li',Inv==1?'active':'']" @click="changeTab(1)">{{$t('login.yxyz')}}</view>
    </view> -->
    <!--register-tab end-->
    <!--register-box start-->
    <view class="register-box" v-show="Inv == 0">
      ...
      <view class="login-ul">
        ...
        <view class="login-li">
          ...
          <view class="li-input">
            <input class="input" type="password" placeholder-class="color-999" v-model="pwd2" :placeholder="$t('login.qqrmm')" />
          </view>
        </view>
      </view>
      <!--login-ul end-->
...
export default {
  ...
  methods: {
    ...
    onLoginForgetPwdByMobile () {
      ...
      if (!this.pwd2) return uni.showToast({ title: this.$t('login.qsrqrmm'), icon: 'none' })
