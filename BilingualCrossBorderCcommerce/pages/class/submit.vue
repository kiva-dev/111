...
export default {
  ...
  methods: {
    ...
    onIndexGoodsDetail (goods_id, shopCont) {
      this.$http.post(this.$apiObj.IndexGoodsDetail, { goods_id: goods_id }).then(res => {
        if (res.code == 1) {
          if (this.isShopCont) {
      ...
      this.$http.post(this.$apiObj.OrderReferOrder, {
        ...
        if (res.code == 1) {
          ...
          if (isNum == 1) {
            ...
            // 3方支付
            this.$http.post(this.$apiObj.OrderMalaysiaPay, { order_no: res.data.order_no, money: this.total }).then(res => {
              if (res.code == 1) {
    ...
    onPwdClick () {
      ...
      this.$http.post(this.$apiObj.OrderBalancePay, {
        ...
        if (res.code == 1) {
          ...
          if (this.buy_type == 2) {
            uni.navigateTo({
              url: '/pages/index/Psuccess?shopCont=' + JSON.stringify(this.shopCont) + '&userList=' + JSON.stringify(this.userList) + '&order_no=' + this.order_no
            })
          } else {
            uni.navigateTo({
