	...
	<view class="sub-page">
		...
		<view v-for="item in OrderList" :key="item.admin_id" class="sub-box">
			...
			<view class="li-item" v-for="items,k in item.list" :key="k">
				...
				<view class="li-txt">
					...
					<view class="specs">
						<text class="c" @click="onShopClick">
							<block v-for="(i,index) in items.attrs" :key="{index}">
								<block v-if="index!==0">，</block>{{i}}
							</block>
						</text>
						<view class="icon">
			...
			<view class="li-leave">
				...
				<view class="c"><input type="text" v-model="item.remark" :placeholder="$t('order.liuyanxuantian')">
				</view>
			</view>
			<!--li-leave end-->
		</view>
        ...
        <view class="li-r f-22">免运费
          RM<text class="f-26">36.00</text>
			...
			<!-- 拼团不需要优惠券 -->
			<view class="bot-li" v-if="group_type!=1">
			...
			<view class="bot-end">
				...
				<text class="color-red f-22">RM<text
						class="f-28">{{$filter.formatValue(total, 'split')[0]}}</text>.{{$filter.formatValue(total, 'split')[1] ? $filter.formatValue(total, 'split')[1] : '00'}}</text>
			</view>
		</view>
		<!--sub-bot end-->
		<!--sub-bot start-->
		...
		<view class="sub-fixed">
			<view class="fixed-con">
				<view class="t">{{$t('order.heji')}}：<text class="color-red f-bold">RM<text
							class="f-28">{{$filter.formatValue(total, 'split')[0]}}</text>.{{$filter.formatValue(total, 'split')[1] ? $filter.formatValue(total, 'split')[1] : '00'}}
					</text></view>
				<!-- <button class="pay-btn" @click="toggle2('center')">立即支付</button> -->
				<button class="pay-btn"
					@click.stop="$noMultipleClicks(onOrderReferCartOrder)">{{$t('order.ljizhifu')}}</button>
			</view>
		</view>
		<!--sub-fixed end-->
		<!--优惠券选择弹出 start-->
		...
		<uni-popup ref="shopPopup" type="bottom">
			<view class="rule-pop">
				<view class="pop-close" @click="onQueryShop">
					<image class="img" src="../../static/images/close1.png"></image>
				</view>
				<!--pop-hd start-->
				<view class="pop-hd">
				...
				<view class="pop-num">
					...
					<view class="num-coum">
						...
						<input class="coum-input" v-model="number" disabled />
				...
				<!--pop-bot start-->
				<view class="pop-bot">
					<button class="public-btn" @click="onGoPay">{{$t('order.btnsub')}}</button>
				</view>
				<!--pop-bot end-->
			</view>
		</uni-popup>
	...
	export default {
		...
		data() {
			return {
				...
				goods_spec_id: '', // sku的id
				num: '', // 购买数量 
				buy_type: 1, // 1原价购买，2拼团购买
				address_id: '', // 地址id
				addCont: '', // 收货地址
				group_type: 0, // 0普通商品，1发起拼团，2参与拼团
				major_no: '', // 主订单号，当group_type=2时必传
				spec_image: '', // 修改规格图片
				GoodsCont: '', // 商品详情
				attrs: '', // 规格
				price: '', // 选中价格
		...
		onLoad(e) {
			...
			if (e.group_type) {
				this.group_type = e.group_type
				this.buy_type = e.group_type == 1 ? 2 : e.group_type == 2 ? 2 : 1
			}
			if (e.major_no) {
				this.major_no = e.major_no
			}
			let shopCont = JSON.parse(e.shopCont)
			this.shopCont = JSON.parse(e.shopCont)
			// 获取订单列表
			this.onOrderConfirmCartOrder(shopCont)
			this.onIndexGoodsDetail(shopCont.cartdata[0].list[0].goods_id, shopCont)
			// 获取优惠券
		...
		methods: {
			...
			// 获取商品详情
			onIndexGoodsDetail(goods_id, shopCont) {
				this.$http.post(this.$apiObj.IndexGoodsDetail, {
					goods_id: goods_id
				}).then(res => {
					if (res.code == 1) {
						if (this.isShopCont) {
							res.data.goods_name = this.getCaption(res.data.goods_name, 1)
							res.data.spec_list.map(item => {
								item.spec_name = this.getCaption(item.spec_name, 1)
							})
						} else {
							res.data.goods_name = this.getCaption(res.data.goods_name, 0)
							res.data.spec_list.map(item => {
								item.spec_name = this.getCaption(item.spec_name, 0)
							})
						}
						res.data.spec_list.map(item => {
							item.attr.map(items => {
								items.isShow = false
								shopCont.cartdata[0].list[0].attrs.map(itemss => {
			...
			// 获取订单列表
			onOrderConfirmCartOrder(shopCont) {
				shopCont.cartdata.map(item => {
					item.remark = ''
				})
				this.OrderList = shopCont.cartdata
				let num = 0
				...
				this.num = num
				this.total = shopCont.total
				this.totalNum = shopCont.total
				this.goods_spec_id = shopCont.cartdata[0].list[0].goods_spec_id
				this.price = shopCont.cartdata[0].list[0].price // 选中的价格
				this.spec_image = shopCont.cartdata[0].list[0].image // 选中的图片
				this.stock_num = shopCont.cartdata[0].list[0].stock_num // 库存
				this.spec_sku_id = shopCont.cartdata[0].list[0].spec_sku_id // sku属性id组合
				this.goods_spec_id = shopCont.cartdata[0].list[0].goods_spec_id // skuid
				this.attrs = shopCont.cartdata[0].list[0].attrs // 属性值
				this.number = shopCont.cartdata[0].list[0].num // 数量
			},
			// 点击商品规格
			onShopClick() {
				...
				for (let i in this.GoodsCont.sku_list) {
					if (this.GoodsCont.sku_list[i].spec_sku_id == spec_value_id) {
						...
						if (this.group_type == 1) {
							this.price = this.GoodsCont.sku_list[i].line_price // 选中的价格
						} else {
			...
			// 点击规格减
			onJianClick() {
				if (this.group_type == 1 || this.group_type == 2) return
			...
			onJiaClick() {
				if (this.group_type == 1 || this.group_type == 2) return
				if (this.number == this.stock_number) return
				this.number++
			},
			// 点击规格确定按钮
			onGoPay() {
				this.OrderList[0].list[0].price = this.price // 选中的价格
				this.OrderList[0].list[0].image = this.spec_image // 选中的图片
				this.OrderList[0].list[0].stock_num = this.stock_num // 库存
				this.OrderList[0].list[0].spec_sku_id = this.spec_sku_id // sku属性id组合
				this.OrderList[0].list[0].goods_spec_id = this.goods_spec_id // skuid
				this.OrderList[0].list[0].attrs = this.attrs // 属性值
				this.OrderList[0].list[0].num = this.number // 数量 
				if (this.use_limit < this.number * this.price) {
					this.totalNum = this.number * this.price
					this.total = this.number * this.price - this.coupon_num
				} else {
			...
			onJian(items) {
				...
				if (this.use_limit < items.num * items.price) {
					this.totalNum = items.num * items.price
					this.total = items.num * items.price - this.coupon_num
				} else {
					this.total = items.num * items.price
					this.totalNum = items.num * items.price
					this.coupon_num = 0
					this.MineCouponList.map(item => {
						item.isShow = false
			...
			onOrderReferCartOrder() {
				...
				this.$http.post(this.$apiObj.OrderReferOrder, {
					goods_spec_id: this.goods_spec_id, // sku的id
					num: this.OrderList[0].list[0].num, // 购买数量
					coupon_id: this.coupon_id, // 优惠券id
					buy_type: this.buy_type, // 1原价购买，2拼团购买
					address_id: this.address_id, // 地址id
					group_type: this.group_type, // 0普通商品，1发起拼团，2参与拼团
					major_no: this.major_no, // 主订单号，当group_type=2时必传
			...
			onPwdClick() {
				...
				this.$http.post(this.$apiObj.OrderBalancePay, {
					...
					if (res.code == 1) {
						...
						if (this.buy_type == 2) {
							uni.navigateTo({
								url: '/pages/index/Psuccess?shopCont=' + JSON.stringify(this.shopCont) +
									'&userList=' + JSON.stringify(this.userList) + '&order_no=' + this
									.order_no
							})
						} else {
							uni.navigateTo({
