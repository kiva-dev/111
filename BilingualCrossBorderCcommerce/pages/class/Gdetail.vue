<template>
  <view class="detail-page">
    ...
    <view class="detail-big">
      <uni-swiper-dot class="uni-swiper-dot-box" @clickItem=clickItem :info="GoodsCont.images" :current="current" :mode="mode" field="content">
    ...
    <view class="detail-head">
      <view class="head-hd">
        <view class="price">
          RM<text class="f-34">{{$filter.formatValue(price, 'split')[0]}}</text>.{{$filter.formatValue(price, 'split')[1] ? $filter.formatValue(price, 'split')[1] : '00'}}
        </view>
        <view class="detail-follow" @click="onMineFocus(1)">
          <view class="icon">
            <image class="img" v-if="GoodsCont.is_goods_focus==1" src="../../static/images/auc1.png"></image>
    ...
    <view class="detail-four" v-if="JudgeList.length">
      <view class="detail-title">
        <view class="t flex">{{$t('home.shop.pingjia')}}<text class="f-22 m-l-15">{{total}}{{$t('home.shop.tiao')}}</text></view>
      ...
      <button class="four-btn" @click="navClick('appraise?id='+GoodsCont.goods_id)">{{$t('home.shop.chakanpingjia')}}</button>
    </view>
    <!--评价 end-->
    <!--店铺信息 start-->
    <view class="detail-five" v-if="GoodsCont.shop">
      ...
      <view class="five-bd">
        <view class="bd-li" @click="onMineFocus(2)">
          <view class="icon">
            <image class="img" v-if="GoodsCont.shop.is_shop_focus==1" src="../../static/images/detail1.png"></image>
            <image class="img" v-else src="../../static/images/xin.png"></image>
          </view>
    ...
    <uni-popup ref="popup1" type="bottom">
      <view class="rule-pop">
        ...
        <!--pop-bot start-->
        <view class="pop-bot">
          <button class="public-btn" style="background:#FFB202;" @click="onAddClick">{{$t('home.shop.addto')}}</button>
...
export default {
  ...
  methods: {
    ...
    // 商品关注、取消关注
    onMineFocus (e) {
      this.$http.post(this.$apiObj.MineFocus, { type: e, data_id: e == 1 ? this.GoodsCont.goods_id : this.GoodsCont.shop.id }).then(res => {
        if (res.code == 1) {
          if (e == 1) {
    ...
    // 点击加入购物车
    onAddClick () {
      this.$http.post(this.$apiObj.CartAdd, { goods_spec_id: this.goods_spec_id, num: this.num }).then(res => {
        if (res.code == 1) {
          uni.showToast({ icon: 'none', title: this.isShopCont ? 'Successfully added to shopping cart' : '加入购物车成功' })
        }
      })
    },
    // 点击弹框立即购买
