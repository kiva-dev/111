      ...
      <view class="att-goods" v-if="GoodsList.length">
        <navigator :url="'./Gdetail?id='+item.goods_id" hover-class="none" class="goods" v-for="item in GoodsList" :key="item.goods_id">
          <view class="goods-con">
            ...
            <view class="goods-t" style=" height: 64rpx;">{{ item.goods_name }}</view>
            <view class="goods-m">
              <view class="price">
...
export default {
  ...
  onLoad (args) {
    ...
    this.admin_id = args.admin_id ? args.admin_id : ''
  ...
  methods: {
    ...
    // 商品列表
    onIndexGoodsList () {
      this.$http.post(this.$apiObj.IndexGoodsList, { page: this.page, pagenum: this.pagenum, c_id: this.c_id, sort: this.sort, keyword: this.keyword, recom: this.recom, goods_type: this.goods_type, s_id: this.s_id, admin_id: Number(this.admin_id) }).then(res => {
        if (res.code == 1) {
          if (this.isShopCont) {
            res.data.data.map(item => {
              item.goods_name = this.getCaption(item.goods_name, 1) ? this.getCaption(item.goods_name, 1) : item.goods_name
              item.tag_name = this.getCaption(item.tag_name, 1) ? this.getCaption(item.tag_name, 1) : item.tag_name
            })
          } else {
            res.data.data.map(item => {
              item.goods_name = this.getCaption(item.goods_name, 0) ? this.getCaption(item.goods_name, 0) : item.goods_name
              item.tag_name = this.getCaption(item.tag_name, 0) ? this.getCaption(item.tag_name, 0) : item.tag_name
            })
          }
