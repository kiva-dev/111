<template>
  <view class="auct-page">
    <view class="zxjpCont">
      <view v-for="item in newsjpList" :key="item.id" class="li" :class="item.id==newsjpId?'active':''" @click="onZxjpClick(item.id)">{{item.title}}</view>
    </view>
    <view class="auct-new">
...
export default {
  data () {
    return {
      ...
      pagenum: 10,// 每页显示商品数目
      totalPageNum: 0,// 总条数
      newsjpList: [{ id: 1, title: this.$t('auction.jindu') }, { id: 2, title: this.$t('auction.jssj') }, { id: 3, title: this.$t('auction.yuanjia') }, { id: 4, title: this.$t('auction.jingpaijia') }, { id: 5, title: this.$t('auction.zhikazng') },],// 头部
      newsjpId: 1,// 头部id
      jingpaiList: [],// 列表
  ...
  methods: {
    ...
    onAuctionGoodsFromCate () {
      this.$http.post(this.$apiObj.AuctionGoodsFromCate, { sort: this.newsjpId, page: this.page, pagenum: this.pagenum, c_id: Number(this.id) }).then(res => {
        if (res.code == 1) {
          if (this.isShopCont) {
            res.data.data.map(item => {
              item.tags.map(items => {
                items.name = this.getCaption(items.name, 1) ? this.getCaption(items.name, 1) : items.name
              })
              item.goods_mark = this.getCaption(item.goods_mark, 1) ? this.getCaption(item.goods_mark, 1) : item.goods_mark
              item.goods_name = this.getCaption(item.goods_name, 1) ? this.getCaption(item.goods_name, 1) : item.goods_name
            })
          } else {
            res.data.data.map(item => {
              item.tags.map(items => {
                items.name = this.getCaption(items.name, 0) ? this.getCaption(items.name, 0) : items.name
              })
              item.goods_mark = this.getCaption(item.goods_mark, 0) ? this.getCaption(item.goods_mark, 0) : item.goods_mark
    ...
    daojishi (mss) {
      ...
      mss = parseInt(mss / 24);
      let d = mss;
      return d + this.$t('auction.day') + h + this.$t('auction.shi') + m + this.$t('auction.fen') + s + this.$t('auction.miao')
    },
    // 点击竞拍列表
    onJingPai (item) {
      uni.navigateTo({
