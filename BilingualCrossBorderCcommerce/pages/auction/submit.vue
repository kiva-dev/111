  ...
  <view class="sub-page">
    ...
    <view v-for="item in OrderList" :key="item.admin_id" class="sub-box">
      ...
      <view class="li-leave">
        ...
        <view class="c"><input type="text" v-model="remark" :placeholder="$t('order.liuyanxuantian')"></view>
      </view>
      <!--li-leave end-->
    </view>
    <!--商品列表 end-->
...
export default {
  ...
  methods: {
    ...
    onOrderConfirmCartOrder (shopCont) {
      ...
      this.num = num
      console.log(shopCont.total)
      this.total = shopCont.total
      this.totalNum = shopCont.total
      this.goods_id = shopCont.cartdata[0].list[0].goods_id
    ...
    onOrderReferCartOrder () {
      ...
      this.$http.post(this.$apiObj.AuctionorderReferOrder, {
        auction_type: 1,// 1竞拍商品原价购买，2参与竞拍价购买
        ...
        money: this.total,// 总金额
        auction_goods_id: this.goods_id,// 竞拍商品id
      }).then(res => {
        if (res.code == 1) {
          ...
          if (isNum == 1) {
            ...
            // 3方支付
            this.$http.post(this.$apiObj.AuctionorderMalaysiaPay, { order_no: res.data.order_no, money: this.total }).then(res => {
              if (res.code == 1) {
    ...
    // 点击显示优惠券
    onCouponPopupClick () {
      if (!this.MineCouponList.length) return uni.showToast({ title: this.$t('order.nhmyyhq'), icon: 'none' })
