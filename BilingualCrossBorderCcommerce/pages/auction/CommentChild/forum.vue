<template>
	<view class="u-page" catchtouchmove="false">
		<view class="mp-header">
			...
			<!--头部导航 start-->
			<view class="auct-nav">
				<view v-for="(item,navNub) in navList" :key="navNub" class="li" :class="item.id==navId?'active':''"
						...
						<view class="album">
								...
								<view class="album__right">
									...
									<view style="display:flex;flex-direction: column">
										<view class="album__nickname">
											<u--text :text="luckyForumListItem.user.nickname" bold size="17"
												class="album__text"></u--text>
											<view class="item-l-level">
												<view class="level-icon">
													<image src="@/static/images/mine/mine_icon_vip.webp"
														mode="widthFix"></image>
												</view>
							...
							<view class="album__content">
								<view class="start-class">
									<u-rate active-color="#0AC68E" inactive-color="#CCCCCC" readonly minCount='1'
										gutter="8" size='18' :allowHalf='true'
										v-model="luckyForumListItem.the_star"></u-rate>
									<span>{{luckyForumListItem.the_star}}</span>
								</view>
								<u--text margin="6px 2px" :text="luckyForumListItem.comment"></u--text>
								<view class="album__urls">
									<view class="album_url" v-for="(item,index) in luckyForumListItem.images"
										:key="index" @click="showImages=true;imgInfo = item">
										<img :src="item"></img>
									</view>
								</view>
							</view>
							<!-- 中拍期数 -->
							<view class="album__content" style="margin-bottom:40rpx"
								v-if="luckyForumListItem.is_featured && luckyForumListItem.auction_goods_stage_num"
								@click="onJingPai(luckyForumListItem)">
								<view class="start-class">
							...
							<view class="album__bottom">
								<view class="album__bottom__btn">
									<view class="forumComment" @click="childComment(luckyForumListItem)">
										...
										<view class="forumComment__comment">
											{{luckyForumListItem.luckyForumComments.length || 'Comment'}}
										</view>
									</view>
								</view>
							</view>
		...
		<!-- 子评论 -->
		<u-popup :show="popupShow" @close="close" @open="open" mode="bottom" round="20" :closeOnClickOverlay='false'
			:closeable='true'>
			<view class="popup-child album">
				...
				<view style="width: 100%;height: 50vh;overflow: overlay;">
					<view v-for="(commentaryListItem,commentaryNub) in commentaryList" :key="commentaryNub"
						style="width: 100%;box-sizing: border-box;padding: 20px;">
						<!-- 左侧头像 -->
						<view class="album__header">
							<view class="album__right" style="width: 100%;">
								...
								<view style="display:flex;flex-direction: column">
									<view class="album__nickname">
										<u--text :text="commentaryListItem.user.nickname" bold size="17"
											class="album__text"></u--text>
										<view class="item-l-level">
											<view class="level-icon">
						...
						<childForum :commentChild='commentaryListItem' @submit="submit"
							@ChildTocommit='tocommit(commentaryListItem)' />
					</view>
				</view>
			</view>
			<view class="comment-botm">
				<view class="comment-botm-inpit">
					<u--input :placeholder="$t('new.wxs')" border="surround" maxlength="100" v-model="comment"
						:focus='isFocusBool' @blur="isBlurFun" @focus="focusFun()"
						@confirm="sendCommentTwoInfo()"></u--input>
				</view>
				<view class="comment-num">{{comment.length}}/100</view>
			</view>
		</u-popup>
		<inputbox v-show="imMessageFocusBool" :imMessageFocusBool="imMessageFocusBool"
			@updateImMessageFocusBool="updateImMessageFocusBool" @submit="submit"></inputbox>
		...
		<u-overlay :show="showImages" :duration="400" :z-index="999" :opacity="1">
			<view class="show_images">
				...
				<view class="carousel">
					<image :src="imgInfo" mode="widthFix"/>
				</view>
			</view>
		</u-overlay>
