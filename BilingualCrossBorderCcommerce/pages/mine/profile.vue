          ...
          tempFilePaths.forEach((em, i) => {
            uni.uploadFile({
              ...
              success: (uploadFileRes) => {
                ...
                if (res.code == 200) {
                  let imgs = res.data
                  that.$http.post(that.$apiObj.MineChangeAvatar, { avatar: imgs.url }).then(ress => {
                    if (ress.code == 1) {
                      that.$http.post(that.$apiObj.MineInfo).then(resss => {
                        if (resss.code == 1) {
                          that.userCont = resss.data
                          uni.setStorageSync('userCont', resss.data)
                        }
                      })
                    }
                  })
