<template>
  <view class="mh100p pd20 pb100" :style="{height: 100+'%'}">
    ...
    <scroll-view id="scrollview" @scrolltoupper='scrolltoupper' class="chat-window pb100" scroll-y="true" :style="{ height: style.contentViewHeight + 'px' }" :scroll-with-animation="true" :scroll-top="scrollTop">
      <view class="talk-li my20 m-item" v-for="(item, index) in chat" :key="index">
    ...
    <view class="fix-wrap bgfff px30 py20 pb50" :style="{ bottom: height }">
      <u-search placeholder="请输入发送内容" search-icon="edit-pen" action-text="发送" v-model="sentText" @focus="hgit" @blur="blur" @custom="sendMessage('TEXT', { content: sentText })" @search="sendMessage('TEXT', { content: sentText })"></u-search>
    </view>
  </view>
</template>

<script>
// import index from '../../common/kefu';
// export default index;
// import RCIMIWEngine from "@/uni_modules/RongCloud-IMWrapper-V2/js_sdk/RCIMEngine"
import RongIMLib from "@/common/libs/RongIMLib-3.0.7.1-dev.js"
...
let im = RongIMLib.init({
  appkey: 'ik1qhw09iy93p',
  debug: true
}); // 建议全局只初始化一次     //不要忘记接开获取头像失败时候闸阀
export default {
  // computed: {
  //   ...mapState(['conversationList_info', 'conversationList', 'chat_token'])
  // },
  data () {
    return {
      chat: [],
  ...
  methods: {
    ...
    rongyunInit () {
       ...
       */
      //进行连接
      this.$Im.connect({ token: uni.getStorageSync('userCont').rong_token }).then(user => {
