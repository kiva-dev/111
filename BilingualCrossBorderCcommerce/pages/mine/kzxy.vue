	...
	export default {
		...
		onShow() {
			...
			this.$http.post(this.$apiObj.IndexSetting,{
				fields:'en_k_diamond_purchase_agreement,zh_k_diamond_purchase_agreement'
			}).then(res => {
				if (res.code == 1) {
					res.data.auction_agree = this.onHtmlcont(res.data.zh_k_diamond_purchase_agreement)
					res.data.e_auction_agree = this.onHtmlcont(res.data.en_k_diamond_purchase_agreement)
