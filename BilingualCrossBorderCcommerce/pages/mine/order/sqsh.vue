    ...
    <view class="sh-sec2">
      ...
      <view class="sec2-li">
        ...
        <view class="color-red f-22">
          RM<text class="f-28">{{$filter.formatValue(refund_money, 'split')[0]}}</text>.{{$filter.formatValue(refund_money, 'split')[1] ? $filter.formatValue(refund_money, 'split')[1] : '00'}}
        </view>
      </view>
      <view class="sec2-li">
...
export default {
  data () {
    return {
      ...
      leixingName: '',// 类型
      yuanyinList: [{ id: 1, title: this.$t('user.order.sqsh.pcdp'), isShow: false }, { id: 2, title: this.$t('user.order.sqsh.xsyztk'), isShow: false }, { id: 3, title: this.$t('user.order.sqsh.spqh'), isShow: false }, { id: 4, title: this.$t('user.order.sqsh.mjfch'), isShow: false }, { id: 5, title: this.$t('user.order.sqsh.qtyy'), isShow: false }],// 退款原因
      yuanyinName: '',// 原因
      totNum: '',// 最大数量
  ...
  methods: {
    ...
    onOrderReferAfter () {
      ...
      if (!this.num) return uni.showToast({ icon: 'none', title: this.$t('user.order.sqsh.qxztklx') })
      if (!this.reason) return uni.showToast({ icon: 'none', title: this.$t('user.order.sqsh.qxztkyy') })
      if (!this.refund_type) return uni.showToast({ icon: 'none', title: this.$t('user.order.sqsh.qxztksl') })
