<template>
	<view class="sh-page">
		...
		<view class="sh-sec1">
			...
			<view class="order-li">
				<view class="li-hd">
					<view class="hd-fl">
						<view class="icon">
							<image class="img" :src="sangjiaCont.shop_logo" mode="aspectFill"></image>
						</view>
		...
		<view class="sh-sec2">
			<view class="sec2-li">
				<view class="label">{{$t('user.order.sqsh.tklx')}}：</view>
				<view class="li-fr" @click="toggle1('bottom')">
					<text>{{leixingName?leixingName:$t('user.order.sqsh.qxz')}}</text>
					<view class="more">
			...
			<view class="sec2-li" @click="toggle2('bottom')">
				<view class="label">{{$t('user.order.sqsh.tkyy')}}：</view>
				<view class="li-fr">
			...
			<view class="sec2-li">
				<view class="label">{{$t('user.order.sqsh.tksl')}}：</view>
				<view class="cart-num">
			...
			<view class="sec2-li">
				<view class="label">{{$t('user.order.sqsh.tkje')}}：</view>
				<view class="color-red f-22">
					RM<text
						class="f-28">{{$filter.formatValue(refund_money, 'split')[0]}}</text>.{{$filter.formatValue(refund_money, 'split')[1] ? $filter.formatValue(refund_money, 'split')[1] : '00'}}
				</view>
			</view>
			<view class="sec2-li">
				<view class="label">{{$t('user.order.sqsh.lxdh')}}：</view>
				<view class="li-input">
					<input class="input" type="number" placeholder-class="clor-999" v-model="mobile"
						:placeholder="$t('user.order.sqsh.qtxlxdh')" />
				</view>
			</view>
		</view>
		<!--sh-sec2 end-->
					...
					<view class="add-upload">
						<view class="upload-img">
							<view v-if="images.length<9" class="xiangji">
								<image src="/static/images/mine/grade9.png" @click="chooseImage" mode="aspectFill"
									class="update"></image>
							</view>
							<block v-for="(image, index) in images" :key="index">
								<view class="xiangji">
									<image :src="baseUrl+image" :data-src="baseUrl+image" mode="aspectFill"
										@tap="previewImage" class="update"></image>
		...
		<uni-popup ref="yuanyinPopup" type="bottom">
			<view class="sh-pop">
				...
				<view class="sh-tit">{{$t('user.order.sqsh.tkyy')}}</view>
				<view class="sh-tip">{{$t('user.order.sqsh.qxhwfhf')}}</view>
				<view class="sh-ul">
	...
	export default {
		data() {
			return {
				...
				leixingList: [{
					id: 1,
					title: this.$t('user.order.sqsh.thtk'),
					isShow: false
				}, {
					id: 2,
					title: this.$t('user.order.sqsh.jth'),
					isShow: false
				}, {
					id: 3,
					title: this.$t('user.order.sqsh.jtk'),
					isShow: false
				}, {
					id: 4,
					title: this.$t('user.order.sqsh.wyhh'),
					isShow: false
				}], // 退款类型
				leixingName: '', // 类型
				yuanyinList: [{
					id: 1,
					title: this.$t('user.order.sqsh.pcdp'),
					isShow: false
				}, {
					id: 2,
					title: this.$t('user.order.sqsh.xsyztk'),
					isShow: false
				}, {
					id: 3,
					title: this.$t('user.order.sqsh.spqh'),
					isShow: false
				}, {
					id: 4,
					title: this.$t('user.order.sqsh.mjfch'),
					isShow: false
				}, {
					id: 5,
					title: this.$t('user.order.sqsh.qtyy'),
					isShow: false
				}], // 退款原因
				yuanyinName: '', // 原因
				totNum: '', // 最大数量
		...
		methods: {
				...
				uni.uploadFile({
					...
					success: res => {
						...
						if (data.code == 200) {
							...
							uni.showToast({
								title: this.$t('user.order.sqsh.sccg'),
								icon: 'none',
								duration: 1000
							});
						} else {
			...
			// 点击减
			onJian() {
				if (!this.refund_type) return uni.showToast({
					icon: 'none',
					title: this.$t('user.order.sqsh.qxztklx')
				})
				if (!this.num) return uni.showToast({
					icon: 'none',
					title: this.$t('user.order.sqsh.qjsplj')
				})
				if (this.num == 1) return uni.showToast({
					icon: 'none',
					title: this.$t('user.order.sqsh.ydzxsl')
				})
				this.num = this.num - 1
				this.onOrderRefundMoney()
			},
			...
			onJia() {
				...
				if (this.num == this.totNum) return uni.showToast({
					icon: 'none',
					title: this.$t('user.order.sqsh.ydzdsl')
				})
				this.num = this.num + 1
				this.onOrderRefundMoney()
			},
			...
			onOrderRefundMoney() {
				...
				this.$http.post(this.$apiObj.OrderRefundMoney, {
					order_goods_id: this.shopCont.order_goods_id,
					num: this.num,
			...
			onOrderReferAfter() {
				...
				if (!this.refund_type) return uni.showToast({
					icon: 'none',
					title: this.$t('user.order.sqsh.qxztksl')
				})
				if (!this.mobile) return uni.showToast({
					icon: 'none',
					title: this.$t('user.order.sqsh.qsrlxdh')
				})
				if (!this.descri) return uni.showToast({
					icon: 'none',
				...
				this.$http.post(this.$apiObj.OrderReferAfter, {
					...
					refund_type: this.refund_type, // 1退货退款，2仅退货，3仅退款
