	...
	<view class="order-page">
		...
		<view class="order-box" v-if="Inv == 0">
			<view class="order-nav">
				<view v-for="item in pttypeList" :key="item.id" class="li" :class="item.id==type?'active':''"
					@click="onNavClick(item.id)">{{item.name}}</view>
			</view>
			<view class="order-ul" v-if="orderList.length">
				<view v-for="item in orderList" :key="item.id" class="order-li" @click="onShopClick(item)">
						...
						<view class="mid-txt">
							...
							<view class="b-flex">
								<block v-if="item.status==3&&items.back_num==0">
									...
									<button @click.stop="onSengqing(items,item)"
										class="order-btn gray">{{$t('user.order.sqshh')}}</button>
								</block>
								<block v-else>
									<view class="s">{{items.goods_attrs}}</view>
									<view class="price">
					...
					<view class="order-price" v-if="item.status==2||item.status==3||item.status==4||item.status==5">
						<view></view>
						<view class="shopCont">
							{{$t('user.order.sfk')}} RM<text
								class="f-28">{{$filter.formatValue(item.pay_price, 'split')[0]}}</text>.{{$filter.formatValue(item.pay_price, 'split')[1] ? $filter.formatValue(item.pay_price, 'split')[1] : '00'}}
							<view class="yhje" v-if="item.coupon_price!='0.00'"> {{$t('user.order.yhje')}} RM<text
									class="f-28">{{$filter.formatValue(item.coupon_price, 'split')[0]}}</text>.{{$filter.formatValue(item.coupon_price, 'split')[1] ? $filter.formatValue(item.coupon_price, 'split')[1] : '00'}}
							</view>
						</view>
					</view>
					<!--待付款-->
					<view class="order-bd" v-if="item.status==0">
						<button class="order-btn gray"
							@click.stop="onQueryClick(item)">{{$t('user.order.qxdd')}}</button>
		...
		<view class="order-box" v-if="Inv == 1">
			<!--order-nav start-->
			<view class="order-nav">
				<view v-for="item in pintuantypeList" :key="item.id" class="li" :class="item.id==type?'active':''"
					@click="onNavClick(item.id)">{{item.name}}</view>
			</view>
			<!--order-nav end-->
			<view class="order-ul" v-if="orderList.length">
		...
		<view class="order-box" v-if="Inv == 2" style="display:none;">
			<!--order-nav start-->
			<view class="order-nav">
				<view v-for="item in jingpaitypeList" :key="item.id" class="li" :class="item.id==type?'active':''"
					@click="onNavClick(item.id)">{{item.name}}</view>
			</view>
			<!--order-nav end-->
			<view class="order-ul" v-if="orderList.length">
		...
		<view class="order-box" v-if="Inv == 2">
            ...
            <view class="name" style="background: #eef8e8;color: #7ac756;" v-if="item.select_way==2">{{$t('zhongpai.linghuo')}}</view> -->
						<view style="display: flex;">
							<button class="order-btn gray" v-if="item.isShow&&item.is_complain==0"
								@click.stop="onShenshuClick(item)">{{$t('zhongpai.shenshu')}}</button>
							<button class="order-btn gray" @click.stop="onShenshu(item)"
								v-if="item.see_complain==1">{{$t('zhongpai.ckss')}}</button>
							<button v-if="item.status===4" @click.stop="onshopClick(item,2)" class="order-btn red"
								style="background:rgb(255, 78, 47);color:#fff;border:none">{{$t('zhongpai.qrsh')}}</button>
							<button v-if="item.status===5" @click.stop="$noMultipleClicks(onquerenClick(item))"
								class="order-btn red"
								style="background:rgb(255, 78, 47);color:#fff;border:none">{{$t('zhongpai.queren')}}</button>
						</view>
					</view>
					<!--order-bd end-->
				</view>
				...
				<view class="pop-con">
					...
					<view class="pop-b">
						...
						<button class="pop-btn red"
							@click.stop="$noMultipleClicks(onOrderNotpayCancelOrder)">{{$t('user.order.btnsub')}}</button>
					</view>
				</view>
			</view>
		</uni-popup>
			...
			<view class="public-pop">
				<view class="pop-con">
					...
					<view class="pop-b">
						...
						<button class="pop-btn red"
							@click.stop="$noMultipleClicks(onOrderDeleteOrder)">{{$t('user.order.btnsub')}}</button>
					</view>
				</view>
			</view>
		</uni-popup>
			...
			<view class="public-pop">
				<view class="pop-con">
					...
					<view class="pop-b">
						...
						<button class="pop-btn red" v-if="num===1"
							@click.stop="$noMultipleClicks(onOrderReceiptOrder)">{{$t('user.order.btnsub')}}</button>
						<button class="pop-btn red" v-if="num===2"
							@click.stop="$noMultipleClicks(onAuctionorderConfirmOrder)">{{$t('user.order.btnsub')}}</button>
					</view>
				</view>
			</view>
		</uni-popup>
					...
					<view class="pop-c">
						<view class="pay-input">
							<input class="input" type="password" placeholder-class="color-999" v-model="pay_pwd"
								:placeholder="$t('user.order.qsrzfmm')" />
						</view>
					</view>
					<view class="pay-bot">
						<button class="pay-btn"
							@click="$noMultipleClicks(onPwdClick)">{{$t('user.order.btnsub')}}</button>
					</view>
				</view>
			</view>
		</uni-popup>
		...
		<uni-popup ref="popup1" type="bottom">
			<view class="mode-pop">
				...
				<view v-for="item,k in orderPayList" :key="item.id" class="mode-li">
					<view class="label">
						{{item.title}}
						<block v-if="item.id==1">（{{$t('user.order.kyye')}}<text
				...
				<view class="mode-bot">
					<button class="public-btn"
						@click="$noMultipleClicks(onPayClick)">{{$t('user.order.btnsub')}}</button>
				</view>
			</view>
		</uni-popup>
		<!--支付方式弹出 end-->
...
-----END PUBLIC KEY-----`
	export default {
		data() {
			return {
				...
				pttypeList: [{
					...
					id: 5,
					name: this.$t('user.order.daipingjia')
				}, {
					id: 6,
				...
				pintuantypeList: [{
					...
					id: 3,
					name: this.$t('user.order.daipintUAN')
				}, {
					id: 4,
					name: this.$t('user.order.daifahuo')
				}, {
				...
				orderPayList: [{
					id: 1,
					title: this.$t('user.order.yuerzfu'),
					isShow: false
				}, {
					id: 2,
					title: this.$t('user.order.sfzf'),
					isShow: false
				}, ],
		...
		methods: {
			...
			onPayClick() {
				if (!this.orderCont) return uni.showToast({
					icon: 'none',
					title: this.$t('user.order.qxzzffs')
				})
				if (this.orderCont == 1) {
					// 余额支付弹框
					this.$refs.popup1.close()
			...
			onOrderOrderList() {
							...
							if (this.isShopCont) {
								res.data.data.map(item => {
									item.goods.map(items => {
										items.goods_name = this.getCaption(items.goods_name, 1) ?
											this.getCaption(items.goods_name, 1) : items.goods_name
										items.goods_attrs = this.getCaption(items.goods_attrs, 1) ?
											this.getCaption(items.goods_attrs, 1) : items
											.goods_attrs
									})
								})
							} else {
								res.data.data.map(item => {
									item.goods.map(items => {
										items.goods_name = this.getCaption(items.goods_name, 0) ?
											this.getCaption(items.goods_name, 0) : items.goods_name
										items.goods_attrs = this.getCaption(items.goods_attrs, 0) ?
											this.getCaption(items.goods_attrs, 0) : items
											.goods_attrs
									})
								})
							}
				...
				// 拼团订单
				if (this.Inv == 1) {
					this.$http.post(this.$apiObj.OrderGroupOrderList, {
						page: this.page,
						pagenum: this.pagenum,
						type: this.type
					}).then(res => {
					...
					if (res.code == 1) {
						res.data.data.map(item => {
							...
							if (item.status == 2) {
								...
								console.log('11:', this.$filter.to_date_time(shop_send_days), this.$filter
									.to_date_time(item.select_time));
							...
							if (item.status == 3) {
								...
								console.log(this.$filter.to_date_time(shop_send_days), this.$filter
									.to_date_time(item.send_time));
								if (shop_send_days * 1000 < new Date().getTime()) {
									isShow = true
								} else {
									isShow = false
			...
			// 删除订单接口
			onOrderDeleteOrder() {
				this.$http.post(this.$apiObj.OrderDeleteOrder, {
					order_no: this.order_no
				}).then(res => {
					if (res.code == 1) {
						uni.showToast({
							title: this.$t('user.order.scddcg'),
							icon: 'none'
						})
						this.page = 1
						this.orderList = []
