  ...
  <view class="width-page">
    ...
    <view class="width-box">
      ...
      <view class="width-hd">
        ...
        <input class="input" type="number" placeholder-class="color-999" v-model="money" :placeholder="$t('user.recharge.qsrczje')" />
      </view>
      <!--width-hd end-->
      ...
      <view class="width-bot">
        <button class="public-btn" @click.stop="$noMultipleClicks(onRechargeAddMoney)">{{$t('user.recharge.ljcz')}}</button>
      </view>
      <!--width-bot end-->
    </view>
    <!--width-box end-->
...
export default {
  ...
  methods: {
    ...
    onRechargeAddMoney () {
      if (!this.money) return uni.showToast({ title: this.$t('user.recharge.ljcz'), icon: 'none' })
      if (+this.money < 10) return uni.showToast({ title: this.$t('monthNum'), icon: 'none' })
      if (Number.isInteger(+this.money) === false) return uni.showToast({ title: this.$t('monthNums'), icon: 'none' })
      this.$http.post(this.$apiObj.RechargeAddMoney, { money: this.money }).then(res => {
        if (res.code == 1) {
