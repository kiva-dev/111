				...
				<view class="pop-con">
					...
					<view class="pop-b">
						...
						<button class="pop-btn red" style="background: #FF4E2F;"
							@click="clearCache">{{$t('user.xitong.btnsub')}}</button>
					</view>
				</view>
			</view>
		</uni-popup>
			...
			<view class="public-pop">
				<view class="pop-con">
					...
					<view class="pop-b">
						...
						<button class="pop-btn red" style="background: #1DD181;"
							@click.stop="$noMultipleClicks(onMineLoginOut)">{{$t('user.xitong.btnsub')}}</button>
					</view>
				</view>
			</view>
		</uni-popup>
		...
		methods: {
			...
			onMineCancelAccount() {
				this.$http.post(this.$apiObj.MineCancelAccount).then(res => {
					if (res.code == 1) {
						uni.showToast({
							title: this.isShopCont ? 'Logout succeeded' : '注销成功',
							icon: 'none'
						})
