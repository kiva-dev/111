  ...
  <view class="width-page">
    ...
    <view class="width-box">
      ...
      <view class="width-hd" style="margin-bottom:0;">
        ...
        <input class="input" placeholder-class="color-999" v-model="moneys" :placeholder="$t('user.drawal.qsrtxye')" />
        <button class="hd-btn" @click="onQuanbu">{{$t('user.drawal.qbtx')}}</button>
      </view>
      <view style="margin-bottom:40rpx;color:red;margin-left:80rpx">{{$t('tixiansxf')}}{{user_tocash_rate}}%</view>
      <!--width-hd end-->
      <view class="width-t">{{$t('user.drawal.txfs')}}</view>
      <!--width-li start-->
      <view class="width-li">
        <view class="label">{{$t('user.drawal.ylzf')}} <block v-if="CardCont">（{{CardCont.account.substr(0,4)}}****{{CardCont.account.substr(-3)}}）</block>
        </view>
        <view class="li-r">
          <text class="color-red" v-if="CardCont" @click="navClick('bank')">{{$t('user.drawal.qhyhk')}}</text>
          <text class="color-red" v-else @click="navClick('bank')">{{$t('user.drawal.yhklb')}}</text>
      ...
      <view class="width-bot">
        <button class="public-btn" @click.stop="$noMultipleClicks(onRechargeTocash)">{{$t('user.drawal.ljtx')}}</button>
      </view>
      <!--width-bot end-->
    </view>
    <!--width-box end-->
...
export default {
  ...
  methods: {
    ...
    onRechargeTocash () {
      ...
      if (!this.moneys) return uni.showToast({ title: this.$t('user.drawal.qsrtxye'), icon: 'none' })
      if (this.moneys > Number(this.money)) return uni.showToast({ title: this.$t('user.drawal.txjebndyktxje'), icon: 'none' })
      this.$http.post(this.$apiObj.RechargeTocash, { money: this.moneys, bank_id: this.CardCont.id }).then(res => {
        if (res.code == 1) {
          uni.navigateTo({
            url: './Wsuccess'
          })
