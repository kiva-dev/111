  ...
  <view class="phone-page">
    <view class="phone-box">
      <view class="li">
        ...
        <view class="li-right">
          <input class="input" v-model="mobile" placeholder-class="color-999" :placeholder="$t('user.phone.newphone')" />
        </view>
      </view>
      <view class="li">
        <view class="li-left">{{$t('user.phone.yzm')}}</view>
        <view class="li-right">
          <input class="input" v-model="code" placeholder-class="color-999" :placeholder="$t('user.phone.qsryzm')" />
        </view>
        <button class="ver-btn" v-if="codeTxt1==$t('user.phone.hqyzm')" @click.stop="$noMultipleClicks(onLoginSendMobileCode)">{{$t('user.phone.fsyzm')}}</button>
        <button class="ver-btn" v-else>{{codeTxt1}}</button>
      </view>
    </view>
    <view class="phone-bot">
      <button class="public-btn" @click="onMineChangeMobile">{{$t('user.phone.qrxg')}}</button>
    </view>
  </view>
</template>
<script>
export default {
  ...
  methods: {
    ...
    onLoginSendMobileCode () {
      if (!this.mobile) return uni.showToast({ title: this.$t('user.phone.qsrphone'), icon: 'none' })
      if (this.mobile_area_code == 86) {
        if (this.mobile) {
          var reg_tel = /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/ //11位手机号码正则
          if (!reg_tel.test(this.mobile)) return uni.showToast({ title: this.$t('user.phone.qsrzqsjh'), icon: 'none' })
        }
      }
      this.$http.post(this.$apiObj.LoginSendMobileCode, { mobile: this.mobile, mobile_area_code: this.mobile_area_code.slice(1) }).then(res => {
        if (res.code == 200) {
          uni.showToast({ title: this.$t('user.phone.fscg'), icon: 'none' })
          this.timeDowns()
        }
      })
    },
    ...
    onMineChangeMobile () {
      if (!this.mobile) return uni.showToast({ title: this.$t('user.phone.qsrphone'), icon: 'none' })
      if (!this.code) return uni.showToast({ title: this.$t('user.phone.qsryzm'), icon: 'none' })
      if (this.mobile_area_code == 86) {
        if (this.mobile) {
          var reg_tel = /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/ //11位手机号码正则
          if (!reg_tel.test(this.mobile)) return uni.showToast({ title: this.$t('user.phone.qsrzqsjh'), icon: 'none' })
      ...
      this.$http.post(this.$apiObj.MineChangeMobile, {
        ...
        if (res.code == 1) {
          uni.showToast({ title: this.$t('user.phone.xgcg'), icon: 'none' })
          setTimeout(() => {
