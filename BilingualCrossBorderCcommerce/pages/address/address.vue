<template>
  <view class="address-page">
    ...
    <view class="address-null" v-else>
      <view class="null-img">
        <image class="img" src="../../static/images/null1.png"></image>
      </view>
      <view>{{$t('user.address.zwdz')}}~</view>
    </view>
    <!--address-null end-->
    <!--address-fixed start-->
...
export default {
  ...
  methods: {
    // 地址列表
    onAddressList () {
      this.$http.post(this.$apiObj.AddressList, { page: this.page, pagenum: this.pagenum }).then(res => {
        if (res.code == 1) {
