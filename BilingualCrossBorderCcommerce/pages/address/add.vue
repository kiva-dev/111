<template>
	<view class="add-page">
		<uni-header :title="$t('top.xinzshdz')"></uni-header>
		<view class="add-box">
			<view class="add-li">
			...
			<view class="add-li">
				...
				<view class="li-fr active">
					...
					<view class="li-input">
						<input class="input" type="number" placeholder-class="color-9999" v-model="mobile"
			...
			<view class="add-li">
				...
				<view class="switch">
					<switch :checked="is_default?true:false" color="rgb(10, 198, 142)" style="transform:scale(0.7);"
						@change="switchChange" />
				</view>
			</view>
			<!--add-bot start-->
		...
		methods: {
			...
			onAddClick() {
				...
				this.$http.post(this.$apiObj.AddressAdd, {
					...
					if (res.code == 1) {
						uni.showToast({
							title: this.$t('user.address.tjcg'),
							icon: 'none'
						})
						setTimeout(() => {
							uni.navigateBack({
